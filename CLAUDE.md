...
- **`app/globals.css`** holds the entire brand system keyed to CSS variables (`--paper`, `--ink`, `--signal`, `--steel`, `--rule`, `--pad`) and font variables set by `next/font` in the layout (`--font-display` Anton, `--font-body` Archivo, `--font-mono` IBM Plex Mono). Section components reuse shared class patterns (`.sec-head`, `.svc`, `.mono`, `.mark`, `.btn`).
